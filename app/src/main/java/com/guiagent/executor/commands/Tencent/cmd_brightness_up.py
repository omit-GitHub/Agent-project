# -*- coding: utf-8 -*-
"""腾讯视频 亮度+。支持 count 参数控制调亮格数。

对标 Java: TencentBrightnessUpCommand → tencent.brightness_up

⚠️ 腾讯视频实测不支持侧滑手势调节亮度（详见 readme_tencent.md），
   保留命令以备未来支持。

策略:
  屏幕左侧 25% 处垂直上滑 — 每次滑动调亮一格。

参数:
  {"count": 3} — 调亮 3 格（默认 1，最大 20）
"""
import json
import sys
import time
...
CMD_NAME = "tencent.brightness_up"
...
def run(params=None):
    ...
    Returns:
        dict: {"ok": True,  "data": {"command": "tencent.brightness_up", "result": "brightness_up x3"}}
    ...
    try:
        for k in range(count):
            swipe_result = _swipe_brightness(up=True)
            if not swipe_result.get("ok"):
                return error("SWIPE_FAILED",
                             f"brightness_up swipe #{k + 1} returned false")
            if k < count - 1:
                sleep_ms(400)
        return success(CMD_NAME, f"brightness_up x{count}")
    except Exception as e:
        return error("EXECUTION_FAILED", str(e))


if __name__ == "__main__":
    # CLI 用法: python cmd_brightness_up.py [count]
    p = None
    if len(sys.argv) > 1:
        try:
