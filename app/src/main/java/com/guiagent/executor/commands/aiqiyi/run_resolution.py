...
def run(params=None):
    """Registry 入口 — aiqiyi.set_quality。

    params: {"quality": "720P"} 或 {"values": ["720P"]}
    """
    target = None
    if params:
        target = params.get("quality")
        if not target:
            ...
            if isinstance(values, list) and len(values) > 0:
                ...
                target = values
    if not target:
        return error("BAD_PARAMS", "Missing parameter: quality")
    target = str(target)
    if target not in RESOLUTION_PATTERNS:
        return error("BAD_PARAMS",
                     f"Invalid quality: {target}. Valid: {', '.join(RESOLUTION_PATTERNS.keys())}")
    ...
    # 3. 点击清晰度按钮
    send({"id": "r3", "op": "tap", "args": {"x": resolution_btn[0], "y": resolution_btn[1]}})
    time.sleep(1.0)
    ...
    node = find_node_by_text(patterns)
    if not node:
        return error("NO_MATCH", f"Quality option {target}P not found")
    ...
    mode = "TV" if is_tv else "movie"
    return success("aiqiyi.set_quality", f"quality_{target}P ({mode} mode)")
